class level3 extends Phaser.Scene {
  ...
  create() {
    ...
    this.chilliText = this.add.text(
      100, 100, 'Chilli: 0', { fontSize: '24px', fill: '#ff00ff' }
    ).setScrollFactor(0)
    .setDepth(999);
    ...
    this.enemy1 = this.physics.add.sprite(1226, 1516, 'enemy').play("enemy-down")
    this.tweens.add({
      targets: this.enemy1,
      y: 1386,
      flipY: false,
    ...
    this.enemy4 = this.physics.add.sprite(265, 966, 'enemy').play("enemy-down")
    this.tweens.add({
      targets: this.enemy4,
      y: 850,
      flipY: false,
...
if(chilliCounter >= 5){
  ...
  collectChilli(player, chilli){
      ...
      this.chilliText.setText(`Chilli: ${chilliCounter}`);
